...
class attention1d(nn.Module):
    def __init__(self, in_planes, ratios=0.25, K=4, temperature=34, init_weight=True):
...
class attention2d(nn.Module):
    ...
    def updata_temperature(self): # 未调用
        if self.temperature!=1:
            self.temperature -=3
            print('Change temperature to:', str(self.temperature))
    ...
    def forward(self, x):
        ...
        x = F.relu(x)
        x = self.fc2(x).view(x.size(0), -1)
        return F.softmax(x/self.temperature, 1) # torch.Size([1, 4])
